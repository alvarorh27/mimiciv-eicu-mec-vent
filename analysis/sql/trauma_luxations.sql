...
FROM (
  ...
  FROM (
    ...
    HAVING
      luxation_any_seq=1
      OR luxation_seq_1=1 -- to group when there is LUXATION
    ORDER BY
      subject_id,
      hadm_id,
      luxation_seq_1))
WHERE
  NOT (hadm_id=prev_hadm_id
    AND luxation_seq_1=0
    AND luxation_any_seq=1) --look at word
ORDER BY
  subject_id,
  hadm_id
