...
from vd3),
    ...
    ORDER BY
      ...
      unitvisitnumber,
      ventnum),
    sq3 AS(/*hours OF ventilation grouped BY the same startday and stay_id, filtered >6 hours. 44632 ROWS grouped BY stay_id
      AND startday*/
    SELECT
      subject_id,
      hadm_id,
      ...
      SUM(hours_of_vent) AS total_vm_hours
    FROM
      sq2
    GROUP BY
      subject_id,
    ...
    ORDER BY
      subject_id,
      startday),
		sq4 AS(/*group by stay_id. 42586 rows*/
  SELECT
    subject_id, 
		hadm_id,
		stay_id,
