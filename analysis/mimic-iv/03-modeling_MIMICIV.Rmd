...
logreg_hospdeath <- glm(
   inhospital_death ~ age + sex + prolonged_mec_vent + sapsii + charlson,
...
htmlTable(or_hospdeath_2[,c("term","OR","OR_lower","OR_upper")], caption = "Odds Ratio and Confidence Intervals for Hospital Mortality-associated Factors. MIMIC-IV Database")
...
or_table <- or_hospdeath_2 %>%
  ...
  arrange(desc(term)) %>%
  kbl(caption = "Odds Ratio and Confidence Intervals for Hospital Mortality-associated Factors. MIMIC-IV Database" , align = "c") %>%
...
print(Sys.time())
logreg_hospdeath <- glm(
   inhospital_death ~ age + sex + trauma_type_anyseq + sapsii + charlson,
  data = final_dataset_MIMICIV,
  family = binomial())
print(Sys.time())
```
...
print(Sys.time())
logreg_hospdeath <- glm(
   inhospital_death ~ prolonged_mec_vent * trauma_type_anyseq + age + sex + sapsii + charlson,
  data = final_dataset_MIMICIV,
  family = binomial())
print(Sys.time())
```
...
ggsave("C:/Users/se_al/Documents/GitHub/mimiciv-mec-vent/figures/mimic-iv/Forest_plot/interactionplot_PMV-TBI_mimiciv.png", width = 6, height = 4, dpi = 300)
