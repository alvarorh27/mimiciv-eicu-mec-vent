WITH
  ...
  SELECT
    *,
    ...
    CASE
      WHEN (((icd_code BETWEEN 'T20' AND 'T29____') OR (icd_code BETWEEN 'T30' AND 'T32_') OR (icd_code BETWEEN '94%' AND '94999')) AND icd_code NOT LIKE '______S' AND icd_code NOT LIKE '______D') AND seq_num=1 THEN 'YES'
    ELSE
    'NO'
  END
    AS burns_seq_1,
...
SELECT
 ...
 FROM (
  ...
  FROM (
    ...
    HAVING
      burns_any_seq='YES'
      OR burns_seq_1='YES' -- to group when there are BURNS
    ORDER BY
      subject_id,
      hadm_id,
      burns_seq_1))
WHERE
  NOT (hadm_id=prev_hadm_id
    AND burns_seq_1='NO'
    AND burns_any_seq='YES') --look at word
ORDER BY
  subject_id,
  hadm_id
