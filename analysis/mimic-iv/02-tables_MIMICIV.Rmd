...
# Function to obtain subset table 1

```{r}
generate_table1_subset <- function(subset_filter, title, save_name) {
  # Subset table
  table1_dataset <- final_dataset_MIMICIV %>%
    filter(trauma_type_anyseq==subset_filter)

  # Variables to include in Table 1
  vars_in_table1 <- c('age', 'icu_stay_days', 'inhospital_death', 'death_year', 'apsiii', 'charlson')
  stratifyby <- "prolonged_mec_vent"
  
  # Subset only needed columns
  table1_dataset <- table1_dataset[, c(vars_in_table1, stratifyby)]
  table1_dataset$prolonged_mec_vent <- factor(table1_dataset$prolonged_mec_vent, labels = c("No", "Yes"))
  ...
  # Save in .docx with custom flextable function
  create_flextable(
    table1 = table1,
    values = c("", "", "PMV", ""),
    colwidths = c(1, 1, 2, 1),
...
## Table one: Subset of non-trauma by PMV
```{r}
generate_table1_subset(
  subset_filter = "non-trauma",
  title = "Baseline characteristics of the MIMIC-IV non-trauma patient subset, stratified by PMV status",
  save_name = "table1_subset_nontrauma_pmv"
)
```

## Table one: Subset of non-TBI trauma by PMV
```{r}
generate_table1_subset(
  subset_filter = "non-TBI trauma",
  title = "Baseline characteristics of the MIMIC-IV non-TBI trauma patient subset, stratified by PMV status",
  save_name = "table1_subset_nonTBItrauma_pmv"
)
```

## Table one: Subset of non-severe TBI by PMV
```{r}
generate_table1_subset(
  subset_filter = "non-severe TBI",
  title = "Baseline characteristics of the MIMIC-IV non-severe TBI patient subset, stratified by PMV status",
  save_name = "table1_subset_nonsevereTBI_pmv"
)
```

## Table one: Subset of severe TBI by PMV
```{r}
generate_table1_subset(
  subset_filter = "severe TBI",
  title = "Baseline characteristics of the MIMIC-IV severe TBI patient subset, stratified by PMV status",
  save_name = "table1_subset_severeTBI_pmv"
)
```
