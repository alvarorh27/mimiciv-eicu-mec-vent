...
### All 3 gcs variables are added together to obtain gcs, only when gcs is NA (530 rows, also with gcs_unable missing), whose gcs is filled completely based on mode; and patients with TBI and gcs_unable=1. Rows with gcs_unable=1 and not TBI has to maintain their 15 value. 
...
##The rest of the NA-containing columns are filled with a '0' value. These include dichotomous variables (inicu_death, death_30days, death_year, tbi_seq1, tbi_anyseq, consecutive_vm_days, mv_consecutive_7days, mv_consecutive_14days, mv_consecutive_21days, mv_total_7days, mv_total_14days, mv_total_21days and prolonged_mec_vent), as well as discontinuous variables related with ventilation (consecutive_vm_days and total_vm_days)
  mutate_at(vars(inhospital_death:ncol(total_dataset)), ~ifelse(is.na(.), 0, .))
...
final_dataset_MIMICIV <- final_dataset_hadmid %>%
  group_by(subject_id) %>%
  arrange (icu_admittime) %>% 
  filter(row_number() == 1) %>%
  arrange(subject_id)
...
rows_excluded_subject_id <- nrow(final_dataset_hadmid)-nrow(final_dataset_MIMICIV)
...
# Export file
```{r}
write.csv(final_dataset_MIMICIV,'C:/Users/se_al/Documents/GitHub/mimiciv-mec-vent/data/mimic-iv/final_dataset_MIMICIV.csv', row.names = F)
```
