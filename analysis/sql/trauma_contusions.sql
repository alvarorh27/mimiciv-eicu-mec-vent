...
FROM (
  ...
  FROM (
    ...
    HAVING
      contusion_any_seq=1
      OR contusion_seq_1=1 -- to group when there is CONTUSION
    ORDER BY
      subject_id,
      hadm_id,
      contusion_seq_1))
WHERE
  NOT (hadm_id=prev_hadm_id
    AND contusion_seq_1=0
    AND contusion_any_seq=1) --look at word
ORDER BY
  subject_id,
  hadm_id
