...
# Fill the NA in tbi_seq_1 to avoid errors in next lines
    tbi_seq_1 = ifelse(is.na(tbi_seq_1), 0, tbi_seq_1),
### All 3 gcs variables are added together to obtain gcs_min, only when gcs_min is NA (530 rows, also with gcs_unable missing), whose gcs_min is filled completely based on mode; and patients with TBI and gcs_unable=1. Rows with gcs_unable=1 and not TBI has to maintain their 15 value. 
    gcs_min = as.integer(ifelse(is.na(gcs_min) | (tbi_seq_1==1 & gcs_unable==1), gcs_motor + gcs_verbal + gcs_eyes, gcs_min)),
